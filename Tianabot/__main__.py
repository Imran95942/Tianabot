...
buttons = [
    ...
    [
        InlineKeyboardButton(text=" Разработчик 👨‍✈️", url=f"https://t.me/{OWNER_USERNAME}"),
    ],
   [
        InlineKeyboardButton(text="𝙎𝙪𝙥𝙥𝙤𝙧𝙩 🫂", url=f"https://t.me/{SUPPORT_CHAT}"),
    ],
    [  
        InlineKeyboardButton(text="➕️ Добавить в группу ➕️", url=f"https://t.me/{BOT_USERNAME}?startgroup=true"),
    ], 
...
───────────────────""".format(
                ...
                reply_markup=InlineKeyboardMarkup(
                 [
                  [InlineKeyboardButton(text="📄 Source", url="https://t.me/isIam07"), 
...
*Note*: Borz Music Bot works on a single merged commands for Music and Video
...
✗ *Borz Database Saved Playlists*:

/createplaylist
- Create Your Playlist on Borz's Server with Custom Name
...
/playplaylist 
- Start playing Your Saved Playlist on Borz Servers.""",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
...
/ping
- Ping the Bot and check Ram, Cpu etc stats of Borz.""",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(text="🔙 𝘽𝙖𝙘𝙠", callback_data="tiana_music")]]
            ),
...
• We also thank all the groups who rely on our Bot for this service, we hope you will always like it: we are constantly working to improve it!""",
        ...
        query.message.edit_text(
            text="""*Borzbot is Now Open Source Bot Project.*
...
*Click below Button to Get Source Code.*""",
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="📄 𝙎𝙤𝙪𝙧𝙘𝙚", url="t.me/isIam07"),                 
                    InlineKeyboardButton(text="🔙 𝘽𝙖𝙘𝙠", callback_data="about_"),
                 ]    
                ]
            ),
...
def main():
    ...
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            dispatcher.bot.sendMessage(f"@{SUPPORT_CHAT}", "Borz 𝙐𝙥𝙙𝙖𝙩𝙚𝙙 𝙎𝙪𝙘𝙘𝙚𝙨𝙨𝙛𝙪𝙡𝙡𝙮✅")
        except Unauthorized:
            LOGGER.warning(
